    ...
    for i in tqdm(range(0, len(sources), batch_size), desc=f"Translating ({likelihood_mode})", unit="batch"):
        ...
        if likelihood_mode == "generation":
            ...
            with torch.inference_mode():
                ...
                # concatenate prompt and generated tokens for the forward pass
                full_input_ids = torch.cat([expanded_input_ids, generated_tokens], dim=-1)
                
                # create full attention mask (1 for text, 0 for padding)
                gen_mask = (generated_tokens != processor.tokenizer.pad_token_id).long()
                full_attention_mask = torch.cat([expanded_attention_mask, gen_mask], dim=-1)

                # set prompt labels to -100
                target_labels = generated_tokens.clone()
                target_labels[target_labels == processor.tokenizer.pad_token_id] = -100

                # run forward pass on full sequences
                outputs_tf = model(
                ...
                # shift logits and labels to align (from end of prompt to second-to-last generated token)
            ...
            # mask out padding tokens so they don't affect the likelihood
            mask = (target_labels != -100)

            # sum log-probs and divide by total amount of tokens, then calculate avg
            sum_scores = torch.sum(token_log_probs * mask, dim=-1)
            token_counts = mask.sum(dim=-1)
